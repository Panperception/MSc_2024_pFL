...
def split_indices(num_cumsum, rand_perm):
    # Creating a list of tuples, where each tuple contains a client id and its corresponding sample indices
    # np.split divides the randomized indices among the clients based on the cumulative sum (num_cumsum).
    client_indices_pairs = [(cid, idxs) for cid, idxs in
                            enumerate(np.split(rand_perm, num_cumsum)[:-1])]

    # Convert the list of tuples into a dictionary for easier access later
...
def balance_split(num_clients, num_samples):
    ...
    # Simply divides total samples by number of clients
...
def dirichlet_unbalance_split(num_clients, num_samples, alpha):
    min_size = 0
    # Ensure each client gets at least 10 samples to avoid extremely uneven distribution
    while min_size < 10:
        # Generate proportions for each client based on a Dirichlet distribution parameterized by alpha
...
def homo_partition(client_sample_nums, num_samples):
    # Create an array of indices representing the samples and shuffle it
...
def samples_num_count(client_dict, num_clients):
    ...
    # Convert the list of tuples into a DataFrame for easier manipulation and readability
